...
const PaymentScheduleCalendar = () => {
  ...
  // Handle date change when a specific date is selected
  ...
  const CustomToolbar = ({ label, onNavigate }) => (
    ...
    <div className="custom-toolbar d-flex justify-content-between align-items-center mt-2 mb-4 px-4">
      <h3 id="current-date " className='dark-purple'>{label} <sub className=" text-color fs-14 fw-400 cursor-pointer" onClick={() => onNavigate('TODAY')}>Today</sub> </h3>
      <div>
  ...
  return (
    <div className='container'>
      ...
      <div className="row custom_row  rounded-4  pt-4 px-2 box-shadow-custom">
        <div className="col-md-12 px-0">
          <Calendar
            ...
            views={['month']} // Specify only month view
            onSelectEvent={handleSelectEvent} // Allow event selection
            date={todayDate} // Keep the calendar's date state synchronized
            components={{
              toolbar: CustomToolbar, // Custom toolbar with label and buttons
            }}
      ...
      <h2 className='fs-25 fw-600 mt-1 mb-4'>{moment().format('D MMMM, YYYY')}</h2>
      ...
      <div className='row mb-5 mt-4  mt-md-0'>
        {dataSubscription?.map((item,index)=>(
          ...
          <div className='ps-2 py-3 calendar-card rounded-4'>
            ...
            <p className='fs-16 fw-400 mt-3 mt-md-5 mb-1'>{item?.subscription_name}</p>
